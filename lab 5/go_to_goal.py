...
async def run(robot: cozmo.robot.Robot):
    ...
    converge = False
    while True:
        odometer = compute_odometry(robot.pose)
        last_pose = robot.pose
        if robot.is_picked_up:
            last_pose = cozmo.util.Pose(0, 0, 0, angle_z=cozmo.util.Angle(degrees=0))
            robot.stop_all_motors()
            await robot.say_text("kidnapped").wait_for_completed()
            await robot.play_anim_trigger(cozmo.anim.Triggers.CodeLabUnhappy).wait_for_completed()
            pf = ParticleFilter(grid)
            converge = False
        markers, attonatedImage = await marker_processing(robot, camera_settings, show_diagnostic_image=False)
        if converge:
            pass
        else:
            dx,dy,dh,converge = pf.update(odometer, markers)
            gui.show_particles(pf.particles)
            gui.show_camera_image(attonatedImage)
            gui.show_mean(dx, dy, dh, converge)
            gui.updated.set()
            robot.stop_all_motors()
            if converge:
                g_x, g_y, g_h = goal
                ...
                await robot.turn_in_place(cozmo.util.degrees(degrees=turn_degree)).wait_for_completed()
                await robot.drive_straight(distance=cozmo.util.distance_mm(np.sqrt(dx**2 + dy**2) * 24), speed=cozmo.util.speed_mmps(75.0)).wait_for_completed()
                await robot.turn_in_place(cozmo.util.degrees(degrees = -turn_degree)).wait_for_completed()
